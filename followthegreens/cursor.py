...
class Cursor:
    ...
    def _mkLine(self):
        ...
        logger.debug(f"tick {round(self.segment.length(), 1)}m in {round(self.delta_tim, 3)}s")
    ...
    def _bearing(self, ratio):
        ...
        s = -1 if self.curr_hdg > self.target_hdg else 1
        return self.curr_hdg if before_turn < 5.0 else self.curr_hdg + ratio * s * abs(self.delta_hdg)
    ...
    def move(self, t: float):
        ...
        self.curr_hdg = self._bearing(ratio=r)
        self.cursor.move(lat=self.curr_pos.lat, lon=self.curr_pos.lon, hdg=self.curr_hdg)
